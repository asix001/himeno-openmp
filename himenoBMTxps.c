...
static int imax = MIMAX-1;
static int jmax = MJMAX-1;
static int kmax = MKMAX-1;
...
initmt()
{
#pragma omp target data map(alloc:a, b, c, p, wrk1, wrk2, bnd)
{
...
#pragma omp target data map(tofrom:a, b, c, p, wrk1, wrk2, bnd) map(to:omega) 
{
  for(int n=0 ; n<nn ; ++n)
  {
    gosa = 0.0;
